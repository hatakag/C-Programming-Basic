...
int main(int argc,char * argv[]) {
  ...
  do {
	...
	if (n>300-sum) {printf("Invalid quantity.Too many not enough\n");break;}
	...
	char c;
	int d;
	printf("Your seat you want to cancel: ");scanf("%c%*c%d",&c,&d);__fpurge(stdin);
	...
	printf("All your booked seat(s) after cancelling in this flight: \n");
        printQueue(A[num],'A');
	printQueue(B[num],'B');
	printQueue(C[num],'C');
	printQueue(D[num],'D');
	...
	char c;
	int d;
	printf("Your seat you want to modify: ");scanf("%c%*c%d",&c,&d);__fpurge(stdin);
	if (c=='A') modQueueAt(d,num,&A[num],&A[num],&B[num],&C[num],&D[num],&E[num],&F[num]);
	else if (c=='B') modQueueAt(d,num,&B[num],&A[num],&B[num],&C[num],&D[num],&E[num],&F[num]);
	else if (c=='C') modQueueAt(d,num,&C[num],&A[num],&B[num],&C[num],&D[num],&E[num],&F[num]);
	else if (c=='D') modQueueAt(d,num,&D[num],&A[num],&B[num],&C[num],&D[num],&E[num],&F[num]);
	else if (c=='E') modQueueAt(d,num,&E[num],&A[num],&B[num],&C[num],&D[num],&E[num],&F[num]);
	else if (c=='F') modQueueAt(d,num,&F[num],&A[num],&B[num],&C[num],&D[num],&E[num],&F[num]);
	else {printf("Invalid seat column (A/B/C/D/E/F)\n");break;}
	//printf("Modified successfully\n");
	printf("All your booked seat(s) after modifying in this flight: \n");
        printQueue(A[num],'A');
	printQueue(B[num],'B');
	printQueue(C[num],'C');
	printQueue(D[num],'D');
